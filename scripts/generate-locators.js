...
function parseLocator(candidate) {
  ...
  return {
    by,
    role,
    heText: by === 'role' ? (loc.value.match(/name:\s*"([^"]*)"/) || [])[1] : loc.value,
  };
}
...
for (const [regionName, blocks] of Object.entries(locales)) {
  ...
  for (const [blockName, elements] of Object.entries(blocks)) {
    for (const elementKey of Object.keys(elements)) {
      ...
      // Skip elements that are actual raw translation pairs
      if (!trans || typeof trans !== 'object' || typeof trans.en !== 'string') continue;

      const candidate = allCandidates.find((c) => {
        const p = parseLocator(c);
        return p.heText === trans.en;
      });
      const parsed = candidate
        ? parseLocator(candidate)
        : { by: 'role', role: elementKey === 'logOut' ? 'button' : 'link' };
